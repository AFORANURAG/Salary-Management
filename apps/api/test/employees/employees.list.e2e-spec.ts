...
describe("Employees list (e2e)", () => {
  ...
  describe("search (q)", () => {
    it("matches partial name, case-insensitively", async () => {
      ...
      const res = await http.get("/employees").query({ q: "hop" });
    ...
    it("matches partial employeeCode and email", async () => {
      ...
      const byCode = await http.get("/employees").query({ q: "ABC" });
      expect(byCode.body.data).toHaveLength(1);
      const byEmail = await http.get("/employees").query({ q: "zoe@" });
    ...
    it("returns an empty page with correct total on no match", async () => {
      ...
      const res = await http.get("/employees").query({ q: "zzzz-no-match" });
  ...
  describe("filters", () => {
    it("filters by a single department", async () => {
      ...
      const res = await http.get("/employees").query({ department: "Finance" });
      expect(res.body.data).toHaveLength(1);
    ...
    it("treats repeated filter values as OR within a field", async () => {
      ...
      const res = await http.get("/employees").query({ country: ["US", "IN"] });
    ...
    it("composes filters with the q search", async () => {
      ...
      const res = await http.get("/employees").query({ q: "Nina", department: "HR" });
      expect(res.body.total).toBe(1);
    ...
    it("filters by employment status (excludes soft-deleted when asked)", async () => {
      ...
      const res = await http.get("/employees").query({ status: "ACTIVE" });
      expect(res.body.total).toBe(1);
  ...
  describe("pagination and sort", () => {
    it("returns correct total independent of the page", async () => {
      ...
      const res = await http.get("/employees").query({ page: 2, pageSize: 25 });
    ...
    it("keeps ordering stable across page boundaries (no dup/skip)", async () => {
      ...
      const p1 = await http.get("/employees").query({ page: 1, pageSize: 20, sort: "name:asc" });
      const p2 = await http.get("/employees").query({ page: 2, pageSize: 20, sort: "name:asc" });
      const ids = new Set([
    ...
    it("honors sort direction", async () => {
      ...
      const asc = await http.get("/employees").query({ sort: "name:asc" });
      const desc = await http.get("/employees").query({ sort: "name:desc" });
      expect(asc.body.data[0].name).toBe("Aaron");
      expect(desc.body.data[0].name).toBe("Zed");
    });

    it("caps pageSize at 100", async () => {
      const res = await http.get("/employees").query({ pageSize: 500 });

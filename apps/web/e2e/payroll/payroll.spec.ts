...
test.describe("Payroll — hub", () => {
  ...
  test("PF02: triggering a run via dialog produces a success entry in the list", async ({ page }) => {
  ...
  test("PF03: attempting to re-run the same period shows a 409 conflict message", async ({ page }) => {
    const emp = await createEmployee();
    await setSalaryStructure(emp.id);
...
test.describe("Payroll — detail", () => {
  test("PF04: /payroll/[period] shows correct summary and results rows", async ({ page }) => {
    const emp = await createEmployee();
    await setSalaryStructure(emp.id);
    // Derive unique period from ms timestamp so each test run uses a fresh period
    const ts = Date.now();

...
test.describe("Auth — login page", () => {
  test("HA-E2E-1: unauthenticated visit to / redirects to /auth/login", async ({ page }) => {
    await page.goto("/");
  ...
  test("HA-E2E-4: authenticated user visiting /auth/login is redirected away", async ({
    page,
    context,
  }) => {
    await loginViaUI(page, ADMIN_EMAIL, ADMIN_PASSWORD);
    ...
    // Now navigate to /auth/login — middleware should let through (it's a public route)
...
test.describe("Auth — setup page", () => {
  test("HA-E2E-5: /auth/setup without token shows static error", async ({ page }) => {
